...
public class Market {
	...
	public static void main(String[] args) {
		...
		//판매자2 생성 간판 "판매자2", 사과50개, 원금1700원, 판매가격 2000원
		Saller s2=new Saller("판매자2", 50, 1700, 2000);
		
		//구매자 주체.
		System.out.println("------판매자1에게 구매------");
		...
		//판매자1에게 5개의 사과사기
		b1.buy(s1, s1.getAPPLE_PRICE()*5);//사과가격*5
		...
		//판매자2에게 10개 사과사기//10개살 돈이 안될때 구매가 안되게하기.
		int sale = s2.getAPPLE_PRICE();
		int money = b1.getMoney();
		if(money>=sale*10) b1.buy(s2, sale*10);
		else System.out.println("구매자의 잔액이 부족하여 사과를 살 수 없습니다.");

		b1.showBuy();//구매자의 현황
		s2.showSale();//판매자2의 현황
